...
module.exports = async (client) => {
    ...
    client.on('interactionCreate', async (interaction) => {
        ...
        if (['accept_application', 'pending_application', 'reject_application'].includes(interaction.customId)) {
            // Extracting user ID from the embed description
            const userTag = interaction.message.embeds[0].description.split(' ')[2]; // Assuming the format is "Application from Username#Discriminator"
            const userId = userTag.replace(/[<@!>]/g, ''); // Extract user ID from mention or plain user ID
